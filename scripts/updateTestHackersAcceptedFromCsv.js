#!/usr/bin/env node
/*
Update hackers statuses to accepted from a CSV file.

Default behavior:
- Uses csv/HackAI Preliminary Sort - Acceptances.csv
- Processes all CSV rows
- Applies updates immediately (no dry run)
- Writes unmatched rows report to csv/unmatched_acceptance_rows.csv
*/
...
const DEFAULT_UNMATCHED_PATH = path.join(CSV_DIR, "unmatched_acceptance_rows.csv");
...
async function main() {
  ...
  console.log(
    `Starting updates for ${args.limit > 0 ? `up to ${args.limit}` : "all"} CSV rows...`
  );
  ...
  for (const row of parsedRows) {
    ...
    if (nameMatches.length === 1) {
      ...
      if (docData.status === "accepted") {
        alreadyAccepted += 1;
        console.log(`[ROW ${row.rowNumber}] Already accepted (doc=${docData.docId}).`);
        continue;
      }
      ...
      try {
        ...
        updatedCount += 1;
        console.log(`[ROW ${row.rowNumber}] Updated to accepted (doc=${docData.docId}).`);
      } catch (err) {
  ...
  console.log(`Updated to accepted: ${updatedCount}`);
